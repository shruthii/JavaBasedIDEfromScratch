...
import java.util.*;
class fra extends Frame implements ActionListener,ItemListener
{
    TextArea ta1,ta2;
...
void menusetup()
{
   try{
    ...
    MenuItem m54=new MenuItem("Servlet Runner");
    MenuItem m55=new MenuItem("IExplorer");
    MenuItem m56=new MenuItem("Ms-Access");
    MenuItem m57=new MenuItem("Oracle8.0");
    ...
    MenuItem m71=new MenuItem("Help");
...
public void actionPerformed(ActionEvent e)
{
    ...
    if (s=="New")
    {
      ...
      if(xx=="Application")
      {
         ...
         FileDialog fd=new FileDialog(new fra(),"Save Application",FileDialog.SAVE);
         fd.setVisible(true);
      ...
      if(xx=="Frame")
      {
         ...
         s1+="\n\n\t}});";
         s1+="\n\tsetVisible(true);\n\t}\n}";
         FileDialog fd=new FileDialog(new fra(),"Save Frame",FileDialog.SAVE);
         fd.setVisible(true);
         setTitle("JPDK 1.0 - " + new File(fd.getFile()) + ".java");
         title=getTitle().substring(11,getTitle().length()-5);
         s1+="\nclass " + title;
      ...
      if(xx=="Applet")
      {
         ...
         s1+="\nimport java.awt.event.*;";
         s1+="\nimport java.applet.*;";    
         FileDialog fd=new FileDialog(new fra(),"Save Applet",FileDialog.SAVE);
         fd.setVisible(true);
         setTitle("JPDK 1.0 - " + new File(fd.getFile()) + ".java");
         title=getTitle().substring(11,getTitle().length()-5);
      ...
      if(xx=="Swing")
      {
         ...
         FileDialog fd=new FileDialog(new fra(),"Save Swings",FileDialog.SAVE);
         fd.setVisible(true);
         setTitle("JPDK 1.0 - " + fd.getFile() + ".java");
         title=getTitle().substring(11,getTitle().length()-5);
         s1+="\npublic class " + title + " extends JApplet";
         s1+="\n{\n\tpublic void init()\n\t{\n";
         s1+="\tContainer cp=getContentPane();";
      ...
      if(xx=="JDBC")
      {
         ...
         s1+="\nimport java.io.*;";
         FileDialog fd=new FileDialog(new fra(),"Save JDBC",FileDialog.SAVE);
         fd.setVisible(true);
         setTitle("JPDK 1.0 - " + fd.getFile() + ".java");
         title=getTitle().substring(11,getTitle().length()-5);
         s1+="\npublic class " + title;
      ...
      if(xx=="Servlet")
      {
         ...
         s1+="\nimport java.io.*;\n";
         FileDialog fd=new FileDialog(new fra(),"Save Servlet",FileDialog.SAVE);
         fd.setVisible(true);
         setTitle("JPDK 1.0 - " + fd.getFile() + ".java");
         title=getTitle().substring(11,getTitle().length()-5);
      ...
      if(xx=="HTML")
      {
         FileDialog fd=new FileDialog(new fra(),"Save HTML",FileDialog.SAVE);
         fd.setVisible(true);
         setTitle("JPDK 1.0 - " + fd.getFile() + ".html");
         String s1="<html>";
         s1+="\n<head>";
   ...
   if(s=="Clear")
   {
    ...
    if(s=="BackColor")
    {
     ...
     d1.setSize(280,220);
     d1.setLayout(null);
     s1=new Scrollbar(0,0,10,0,255);
     s2=new Scrollbar(0,0,10,0,255);
     s3=new Scrollbar(0,0,10,0,255);
    ...
    try{
    Runtime t =Runtime.getRuntime();
    Process p=t.exec("start iexplore");
    p.waitFor();
    int i =p.exitValue();
    }catch(Exception ip){}
    }
...
String result ="C:/Program Files/Internet Explorer/IEXPLORE.EXE  http://127.0.0.1:8080/servlet/"+s1;
try
{
...
{}
    ...
    try{
    Runtime t =Runtime.getRuntime();
    Process p=t.exec("c:/Program files/Microsoft office/Office/MSACCESS.exe");
    p.waitFor();
    int i =p.exitValue();
    }catch(Exception ip){}
    }
